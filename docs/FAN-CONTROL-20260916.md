...
**2026-09-18 确认：显示板 USB 电取自 gb10 整机，因此
负载纹波有两条候选耦合路径——① USB 5V → 显示板 3V3 → tach 上拉参考
（R4/R6）；② 风扇 12V 取自整机电源 → 风扇内部 tach 输出级/地弹。
判别实验：显示板改用独立 USB 电源供电（数据链路走 WiFi 不受影响），
若负载时段 dropout 消失则路径 ① 主导，根治可为显示板独立供电或 VBUS
加磁环；若仍跳动则路径 ② 主导，根治落在 tach 线 RC 滤波/上拉加强。
