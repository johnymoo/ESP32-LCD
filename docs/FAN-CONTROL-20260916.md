...
负载相关性复核（同日下午）：整机 GPU 持续 94-96%、模型连续生成 20 分钟
以上（25-60 tok/s）期间，串口 750+ 条状态更新零 dropout 告警——"GPU
负载 → 跳动"的简单相关不成立。回看时间线，13:02 的跳动发生在拍摄/挪动
设备前后，线缆或排母接触受扰动的嫌疑上升；亦不排除长 uptime 或其它偶发
条件。对策维持：固件保持逻辑兜底显示层，串口 WARN + 负载采样 watcher
延长至一日蹲守，下次告警出现时先核对线缆装配再谈电源耦合。
